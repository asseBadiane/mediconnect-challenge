...
export class UpdatePatientComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.params.subscribe(params => {
      ...
      if (this.fhirId) {
        ...
        this.notificationService.showError('No patient ID provided');
  ...
  onCancel(): void {
    this.router.navigate(['/patients', this.fhirId]);
  }
  ...
  checkIdentifierAvailability(): void {
    ...
    if (identifier && identifier.length >= 3 && identifier !== this.patientData?.identifierValue) {
